...
export async function POST(request: Request) {
  try {
    ...
    // 3. Image Generation Logic (Dual-Mode: Vertex AI or Local ComfyUI)
    let imageBuffer: Buffer | null = null;

    if (configs.vertex_ai_settings.use_vertex_ai) {
      console.log('Routing generation request to Google Cloud Vertex AI Custom Prediction Endpoint...');
      const arrayBuffer = await file.arrayBuffer();
      const fileBuffer = Buffer.from(arrayBuffer);
      const base64Input = fileBuffer.toString('base64');

      // Build workflow JSON schema for Vertex AI prediction endpoint
      const workflow = {
        "3": {
          "class_type": "KSampler",
          "inputs": {
        ...
        "9": {
          ...
          "inputs": {
            "image": "input.png" // Custom container decodes base64 payload to inputs/input.png
          }
        },
        "10": {
      ...
      try {
        const base64Output = await predictVertexAI(workflow, base64Input);
        imageBuffer = Buffer.from(base64Output, 'base64');
      } catch (err: any) {
        console.error('Vertex AI prediction failed:', err);
        throw new Error(`Vertex AI generation failed: ${err.message}`);
      }
    } else {
      ...
      // Fetch output image from ComfyUI
      try {
        ...
        console.error('Failed to fetch generated image from ComfyUI:', fetchError);
      }
    }

    if (!imageBuffer) {
      throw new Error('Failed to retrieve generated image data');
    }

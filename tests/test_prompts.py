...
if __name__ == '__main__':
    ...
    test_cases = [
        ...
        (4474664, 0.20, "red terracotta spanish roof tiles", "CN strength 0.20"),
        (4474664, 0.10, "red terracotta spanish roof tiles", "CN strength 0.10"),
        (4474664, 0.00, "red terracotta spanish roof tiles", "CN strength 0.00"),
        (12345, 0.20, "red terracotta spanish roof tiles", "CN strength 0.20 on Seed 12345"),
        (12345, 0.00, "red terracotta spanish roof tiles", "CN strength 0.00 on Seed 12345"),
    ]
    
    for seed, cn_strength, prompt, name in test_cases:
        print(f"\n--- Testing case: {name} (Seed {seed}, CN {cn_strength}, Prompt '{prompt}') ---")
        try:
            img_bytes = run_inpainting(img_name, seed, cn_strength, prompt)
            mod_pixels, mean_rgb, red_pct = analyze_redness(img_bytes)
            print(f"Result -> Modified: {mod_pixels} | Mean RGB: {mean_rgb} | Reddish%: {red_pct:.2f}%")

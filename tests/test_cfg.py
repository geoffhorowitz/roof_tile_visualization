...
def run_inpainting(uploaded_filename, seed, cfg, controlnet_strength, tile_prompt):
    workflow = {
        ...
        "11": {
            ...
            "inputs": {
                "filename_prefix": config.get("local_paths.save_filename_prefix", "roof_cfg_test"),
                "images": ["10", 0]
            }
        },
        "12": {
...
def analyze_redness(img_data):
    ...
    local_mask_path = config.get("local_paths.local_mask_path", "data/sam_mask.png")
    mask = PIL.Image.open(local_mask_path).convert('L')

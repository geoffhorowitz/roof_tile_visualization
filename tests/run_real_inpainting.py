...
def run_inpainting(uploaded_filename, sam_prompt, controlnet_strength, output_path, tile_prompt=None):
    """Runs the inpainting workflow with specified parameters."""
    if tile_prompt is None:
        tile_prompt = config.get("prompts.default_tile_prompt", "red terracotta spanish roof tiles")
        
    print(f"Running inpainting: SAM='{sam_prompt}', ControlNet={controlnet_strength}, Output={output_path}")
    
    # Construct positive prompt by formatting the prompt template
    positive_template = config.get(
        "prompts.positive_prompt_template",
        "photorealistic house roof inpainting, {prompt}, detailed shingle texture, high quality photography"
    )
    formatted_positive_prompt = positive_template.format(prompt=tile_prompt)
    ...
    workflow = {
        ...
        "6": {
            ...
            "inputs": {
                "text": formatted_positive_prompt,
                "clip": ["4", 1]
            }
        },
        "7": {
        ...
        "11": {
            ...
            "inputs": {
                "filename_prefix": config.get("local_paths.save_filename_prefix", "roof_debug"),
                "images": ["10", 0]
            }
        },
        "12": {
...
if __name__ == "__main__":
    ...
    tile_prompt = sys.argv[4] if len(sys.argv) > 4 else None
    
    # Upload house image from config default path

...
def run_inpainting(uploaded_filename, seed, controlnet_strength, tile_prompt, ckpt_name):
    workflow = {
        "3": {
            "class_type": "KSampler",
            "inputs": {
        ...
        "11": {
            ...
            "inputs": {
                "filename_prefix": "roof_model_test",
                "images": ["10", 0]
            }
        },
        "12": {
...
if __name__ == '__main__':
    ...
    test_cases = [
        # (seed, controlnet_strength, tile_prompt, ckpt_name, name)
        (4474664, 0.45, "red terracotta spanish roof tiles", "sd_xl_base_1.0.safetensors", "Base model - Seed 4474664"),
        (12345, 0.45, "red terracotta spanish roof tiles", "sd_xl_base_1.0.safetensors", "Base model - Seed 12345"),
    ]
    
    for seed, cn_strength, prompt, ckpt, name in test_cases:
        print(f"\n--- Testing case: {name} (Model: {ckpt}) ---")
        try:
            img_bytes = run_inpainting(img_name, seed, cn_strength, prompt, ckpt)
            mod_pixels, mean_rgb, red_pct = analyze_redness(img_bytes)
            print(f"Result -> Mask size: {mod_pixels} | Mean RGB: {mean_rgb} | Reddish%: {red_pct:.2f}%")

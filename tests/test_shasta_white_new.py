...
def run_inpainting(uploaded_filename, seed, prompt):
    workflow = {
      "3": {
        "class_type": "KSampler",
        "inputs": {
...
if __name__ == '__main__':
    ...
    seed = 8641719 # Same seed as image 20 to isolate prompt impact
    
    print(f"Generating roof with prompt: '{new_prompt}' and seed: {seed}")
    img_bytes = run_inpainting(img_name, seed, new_prompt)
    
    output_path = "data/output/test_shasta_white_new.png"
    with open(output_path, "wb") as f:
        f.write(img_bytes)
    print(f"Saved generated image to {output_path}")
    
    pixels, mean_rgb = analyze_pixels(img_bytes)
    print(f"Modified pixels: {pixels}")
